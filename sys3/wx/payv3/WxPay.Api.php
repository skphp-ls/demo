...
class WxPayApi
{
	...
	*	企业向个人转款
	*/
	public static function mchOrder($inputObj, $timeOut = 6)
	{
		$url = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers';
		$inputObj->mchAppid(WxPayConfig::$APPID);//公众账号ID
		$inputObj->mchid(WxPayConfig::$MCHID);//商户号
		//随机字符串	    
		$inputObj->SetNonce_str(self::getNonceStr());
		//终端ip
		$inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);
		$inputObj->checkNameNo();
		$inputObj->desc('提现');	
		//签名
		$inputObj->SetSign();				  
		//$inputObj->SetSpbill_create_ip("1.1.1.1"); 
		...
		$result = WxPayResults::InitMch($response);
		//print_r($result);die;
		//self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
		return $result;
	}
